...
- `codex exec`へ`--sandbox read-only`を渡す。ファイルシステムへの書き込みを許さない。
...
この条件が効くことはCLI 0.154.0で実測した。ファイル書き込みを促す指示文を与えても、
sandboxが書き込みを拒否し、モデルは失敗を認めた文面を返すだけでファイルは作られず、
承認待ちで停止することもなかった。CLIを更新したときは同じ確認をやり直す。

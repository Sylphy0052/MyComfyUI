...
- 人物参照画像は`inputs/`配下だけを許可する。import前に実体または`input_files`のbase64について、実形式、サイズ、SHA-256を照合し、backupからのrestoreでは内容アドレス付き入力cacheへ復元する。画像本体の合計はProject package上限以内に制限する。

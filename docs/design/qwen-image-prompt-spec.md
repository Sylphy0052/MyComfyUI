...
本節は 2026-09-22 に決定した方針を記録する。実装は #173 で行った。
...
この結果、`DEFAULT_VALUES` に入れた baseline がフォームの初期値として見えるのは新しく作った環境だけになる。既存の環境で baseline が効くのは、提案から投入する Job と prompt 補完の応答に限られる。どちらも投入操作や応答を組み立てる時点でマージするからである。既存の環境で画面から手動生成する場合、Recipe の既定値は空文字のままなので baseline は乗らない。利用者が自分で書くか、Recipe を作り直す必要がある。
...
5 節の目安 (512 トークン上限、200 トークン以下が素直、300 超で後方のタグが効かない) を、文字数の制約として次のように置く。トークン数は数えず、文字数で近似する。

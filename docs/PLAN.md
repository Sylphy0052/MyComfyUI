...
|準備|ワークフロー登録案、バッチ生成計画、資産分類|内容確認後に反映。適用先はRecipe登録、生成ジョブの一括投入、Artifactタグの更新|
|実行|生成ジョブ投入、ファイル移動、Git操作、外部送信|対象と差分を表示し、明示承認後に実行|

準備段階のワークフロー登録案は、Workflow本体を増やさない。Workflowの登録簿は起動時に同梱テンプレートとAdapterの実装から組み立て直す派生データであり、行を足しても実行できるWorkflowは増えないためである。登録案の適用先は、案に沿うRecipeを既存のWorkflow版に対して登録することとし、実行できるテンプレートの許可リストは変えない。新しいWorkflow本体が要る案は提案の表示にとどめ、適用の対象にしない。

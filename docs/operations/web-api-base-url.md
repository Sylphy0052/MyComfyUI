...
globalが未定義、文字列でない値、空文字、URLとして読めない値、`http`と`https`以外のscheme、loopback以外のhostのいずれかなら、同一originの`/api/v1`へ寄せる。未定義以外はいずれも理由を`console.warn`へ残す。queryとhashは使わずに捨て、付いていた場合だけ同じく警告を残す。
